...
MediaRemote Adapter。事件流为空或不可用时，应用会由系统 `/usr/bin/osascript` 按需执行
MIT 许可的网易云定向 JXA 查询，并在事件流恢复后停止轮询。这些路径依赖未公开的 macOS
...
和唱片中心；播放、暂停、Reduce Motion、屏幕休眠和会话锁定会控制持续动画。
