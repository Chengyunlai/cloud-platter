...
- 修订：真实使用发现默认事件流在切歌或播放器失去前台后可能持续输出空快照，因此把定向
  JXA 查询从未来备选提升为按需稳定性通道。
- 修订：用户再次观察到播放期间界面停留在空闲状态，而同期一次性 `get` 快照仍可读取媒体；
  因此在 JXA 前增加同一隔离 Adapter 的一次性快照，形成两级自愈。
- 修订：长驻事件流也可能在输出有效状态后保持存活但不再发送事件；因此增加 4 秒静默检测，
  超时后进入同一备用快照链路。
- 修订：首版严格只读的产品边界已由
  [ADR-0005](0005-user-initiated-desktop-playback-controls.md) 调整为三种用户主动播放控制；播放
  状态读取、隐私和隔离边界继续沿用本决策。
...
- 启动时先运行 capability test。系统缺少 `/usr/bin/perl`、私有框架发生变化、测试失败，或
  实时事件流返回空闲/不可用状态，或连续 4 秒没有事件时，先通过同一 `/usr/bin/perl` 隔离
  边界执行一次性 `get` 快照；快照空闲或不可用时，再按需启用由
  `/usr/bin/osascript -l JavaScript` 承载、只查询 `com.netease.163music` 的 JXA 通道。查询到
  活动媒体时每秒更新；无媒体时降为 2 秒，调用失败时降为 4 秒。默认事件流恢复后立即停止备用
  轮询；三条读取路径都失败时显示明确的兼容性提示和默认唱片。
...
- 一次性 `get` 与 JXA 备用路径需要定时查询，实时性和能耗弱于默认事件流；两者同样使用私有
  MediaRemote，不能被描述为 Apple 官方支持的解决方案。轮询只在主流空闲、不可用或静默超过
  4 秒时运行，并使用 1/2/4 秒自适应间隔限制常驻能耗。`get` 复用已有 Adapter 和许可证；随包分发的固定 JXA
  脚本与 MIT 许可证合计增加
  11,392 字节未压缩内容；必要性来自已复现的默认事件流空快照故障，长期维护成本是升级固定
  提交前重新检查许可证、输出协议、兼容性和能耗。
