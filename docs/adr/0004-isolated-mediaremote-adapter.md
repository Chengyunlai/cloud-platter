...
- 启动时先运行 capability test。系统缺少 `/usr/bin/perl`、私有框架发生变化、测试失败，或
  实时事件流返回空闲/不可用状态时，按需启用由 `/usr/bin/osascript -l JavaScript` 承载、只
  查询 `com.netease.163music` 的 JXA 通道。查询到活动媒体时每秒更新；无媒体时降为 2 秒，
  调用失败时降为 4 秒。默认事件流恢复后立即停止轮询；两条路径都失败时显示明确的兼容性
  提示和默认唱片。
...
- JXA 备用路径需要定时查询，实时性和能耗弱于默认事件流；它同样使用私有 MediaRemote，
  不能被描述为 Apple 官方支持的解决方案。轮询只在主流空闲或不可用时运行，并使用
  1/2/4 秒自适应间隔限制常驻能耗。随包分发的固定 JXA 脚本与 MIT 许可证合计增加
